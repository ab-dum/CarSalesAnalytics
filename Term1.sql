...
*********************************************************************
                         TERM PROJECT 1
*********************************************************************
		PART I-CREATING THE TABLES AND LOADING THE DATASET
*********************************************************************
*/
...
-- Creating the tables: category, customer, employee, orders, cars, provider and store.
...
BEGIN
	...
	-- Archiving the all employees and connected table entries to employee_performance table.
...
DELETE FROM cars WHERE car_id=12605;
DELETE FROM category WHERE category_id='XBD';
DELETE FROM orders WHERE order_id='O32456';
CALL EmployeePerformanceStore();

-- Inserting new values to check trigger
...
CALL EmployeePerformanceStore();
DELETE FROM employee_performance WHERE EmployeeId=3 AND Unit=4;
...
-- Testing the number of rows of view `emp_poland_return10000'
SET @expected_row_count = 6;
SET @actual_row_count = (select count(*) from emp_poland_return10000);

SELECT
CASE
WHEN @expected_row_count = @actual_row_count THEN 'True'
ELSE 'False'
END AS row_check;
...
-- Deleting the inserts
DELETE FROM car_sales WHERE SalesId='1';

-- DATA MARTS

-- This data mart shows the sold cars whose brand is Ford and have price more than 20000 dollars.
...
-- Testing the number of rows of view `countries_soldmorethan8_januarytomarch'
SET @expected_row_count = 28;
SET @actual_row_count = (select count(*) from `countries_soldmorethan8_januarytomarch;`);
